...
注意，我们构建数值型与类别型数据的组合特征主要目的是为了**捕捉数据中潜在的相互作用和关联性**。我们期望这些组合特征可以为提供模型更丰富的信息，帮助模型更好地理解数据。前面，构建类别型数据之间的组合特征时，我们采用先组合后编码的方法。这种方式通常用于捕捉不同类别特征间的潜在相互作用。但是，对于类别型数据和数值型数据的组合，处理逻辑有所不同。当将一个类别型特征和一个数值型特征进行组合时，通常是要根据类别特征的不同水平来调整数值特征的影响。因此，我们需要在组合前对类别型数据进行先编码/转换处理。如果类别型特征未经编码直接与数值型特征结合，你将无法直接通过乘法或其他数值操作来实现**捕捉数据中潜在的相互作用和关联性**的目的。

例如，假设我们有一个类别型特征 `Color`，取值为 "Red"、"Blue" 和 "Green"，以及一个数值型特征 `Size`。如果我们想为每种颜色创建一个基于大小的特征，直接组合未编码的 `Color` 和 `Size` 显然无法达到此目的。但如果我们先对 `Color` 进行编码，比如 "Red" 编码为 1，"Blue" 编码为 2，"Green" 编码为 3，这种直接数值上的组合也不会提供有意义的相互作用信息。这是因为类别型特征的数值编码通常是任意的，它们的数值大小没有实际意义。在这个例子中，颜色被编码为1、2、3，但这些数字本身并不代表颜色之间的任何自然排序或量化差异。因此，将这些编码乘以 `Size` 特征不会给出我们想要的、按颜色区分的大小影响。正确的方法是为每种颜色创建独立的大小特征，这可以通过类别型特征的独热编码与数值型特征的相乘来实现。具体来说：
...
2. **组合特征的创建**：对于数值型和类别型特征的组合，一种常见的方法是通过创建交互项。例如，可以为每个船舱等级创建一个票价的变体，如 `Fare_Pclass_1`，`Fare_Pclass_2`等。这意味着根据船舱等级分别对票价进行分组，并在相应的新特征中填充该分组内的票价值。
